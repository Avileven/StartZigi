...
const getFirmColor = (firmId, vcMeetings) => {
  ...
  if (meeting.status === 'screening_rejected' || meeting.status === 'missed') return 'bg-red-500';
  if (meeting.status === 'screening_passed' && !meeting.meeting_scheduled_at) return 'bg-purple-300';
  if (meeting.status === 'meeting_scheduled' || (meeting.status === 'screening_passed' && meeting.meeting_scheduled_at)) return 'bg-purple-600';
...
const VCFirmContactModal = ({ firm, venture, vcMeetings }) => {
  ...
  const alreadyApplied = existingMeeting && ['pending_screening', 'screening_passed', 'meeting_scheduled', 'meeting_completed'].includes(existingMeeting.status);
