            ...
            <nav className="fixed top-0 left-0 right-0 bg-gray-900/80 backdrop-blur-md z-50 border-b border-white/10">
                    ...
                    <div className="flex items-center justify-between h-20">
                        ...
                        <div className="hidden md:flex items-center space-x-8">
                            <div className="flex items-center space-x-4 border-r border-white/10 pr-4">
                                <Link href="/why-startzig" className="text-white bg-gray-800/50 px-3 py-2 rounded-md text-sm font-medium">
                                    Why StartZig
                                </Link>
                ...
                {isMenuOpen && (
                    <div className="md:hidden bg-gray-900 border-t border-white/10 px-4 pt-2 pb-6 space-y-1">
                        <Link href="/why-startzig" onClick={() => setIsMenuOpen(false)} className="block text-gray-300 py-3 font-medium">Why StartZig</Link>
                        <Link href="/community" onClick={() => setIsMenuOpen(false)} className="block text-gray-300 py-3 font-medium">Community</Link>
                        <Link href="/pricing" onClick={() => setIsMenuOpen(false)} className="block text-gray-300 py-3 font-medium">Pricing</Link>
                        <div className="pt-4 border-t border-white/10 flex flex-col space-y-3">
                            {user ? (
                                <Link href="/dashboard" onClick={() => setIsMenuOpen(false)} className="w-full bg-indigo-600 text-white py-3 rounded-md text-center">Go to dashboard</Link>
                            ) : (
                                <>
                                    <button onClick={handleLogin} className="w-full text-white bg-gray-800 py-3 rounded-md">Login</button>
                                    <Link href="/register" onClick={() => setIsMenuOpen(false)} className="w-full bg-indigo-600 text-white py-3 rounded-md text-center">Sign Up</Link>
                                </>
                            )}
                        </div>
                    </div>
