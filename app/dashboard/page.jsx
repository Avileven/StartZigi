  ...
  const runScreeningCheck = async (venture) => {
    try {
     ...
     // if (hoursElapsed < 0) continue; // TESTING: immediate — replace with line below for production
       if (hoursElapsed < 1) continue; // PRODUCTION
...
if (currentPhaseIndex >= PHASES_ORDER.indexOf('business_plan')) {
  ...
  if (isMobile && !showToS) {
    return <MobileHome venture={currentVenture} messages={messages} userEmail={user?.email} liveBalance={liveBalance} currentValuation={currentValuation} />;
  }
