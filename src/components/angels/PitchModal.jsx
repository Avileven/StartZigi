...
export default function PitchModal({ investor, venture, isOpen, onClose }) {
  ...
  const loadQuestions = useCallback(async () => {
    ...
    try {
      ...
      console.log("Business plan loaded:", plan);
      ...
      // שאלות דינמיות מבוססות business plan
      const dynamicQuestions = [];
      
      if (plan?.solution) {
        dynamicQuestions.push({
          question_id: 'SOLUTION_DEPTH',
          question_text: `Regarding your solution: ${plan.solution.substring(0, 200)}... What is the biggest technical challenge you anticipate in building this specifically, and how will you overcome it?`
        });
      }
      
      if (plan?.target_customers) {
        dynamicQuestions.push({
          question_id: 'SCALABILITY_TARGETING',
          question_text: `You specified your target customers as ${plan.target_customers.substring(0, 150)}... How do you plan to scale your acquisition of these specific users during the first 12 months?`
        });
      }
      ...
      // סידור סופי: פתיחה → דינמיות → רגילות+competitor
      const finalQuestions = [openingQuestion, ...dynamicQuestions, ...questionsWithCompetitor];
      
      // עדכון index של competitor
      competitorQuestionIndexRef.current = insertPosition + 1 + dynamicQuestions.length;
